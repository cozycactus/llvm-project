...
AVR32_LINUX_CACHE=${AVR32_LINUX_CACHE:-$HOME/Library/Caches/avr32-linux}
LINUX_SRC=${LINUX_SRC:-$(first_existing "$BASE/linux" "$BASE/linux-avr32" || true)}
LINUX_SRC=${LINUX_SRC:-$BASE/linux}
LINUX_LLVM_BUILD=${LINUX_LLVM_BUILD:-$(first_existing \
  "$AVR32_LINUX_CACHE/build-llvm-shell" \
  "$BASE/linux-avr32-build-llvm-shell" || true)}
LINUX_LLVM_BUILD=${LINUX_LLVM_BUILD:-$AVR32_LINUX_CACHE/build-llvm-shell}
LINUX_GCC_BUILD=${LINUX_GCC_BUILD:-$(first_existing \
  "$AVR32_LINUX_CACHE/build-gcc-shell" \
  "$BASE/linux-avr32-build-gcc-shell" || true)}
LINUX_GCC_BUILD=${LINUX_GCC_BUILD:-$AVR32_LINUX_CACHE/build-gcc-shell}
ROOTFS=${ROOTFS:-$(first_existing \
  "$LINUX_SRC/tools/avr32-rootfs" \
  "$BASE/avr32-linux-rootfs" || true)}
ROOTFS=${ROOTFS:-$LINUX_SRC/tools/avr32-rootfs}
QEMU=${QEMU:-$(first_existing \
  "$BASE/qemu-avr32-v11-sync/build/qemu-system-avr32" \
  "$BASE/qemu/build/qemu-system-avr32" || true)}
QEMU=${QEMU:-$BASE/qemu-avr32-v11-sync/build/qemu-system-avr32}
HOST_SHIM=${HOST_SHIM:-$(first_existing \
  "$LINUX_SRC/tools/macos-host/include" \
  "$BASE/linux-avr32-host/include" || true)}
HOST_SHIM=${HOST_SHIM:-$LINUX_SRC/tools/macos-host/include}
...
actions:
  ...
  rootfs        Rebuild the embedded AVR32 initramfs root shell.
...
rootfs_initramfs_list() {
  if [[ -f "$ROOTFS/build/initramfs.list" ]]; then
    echo "$ROOTFS/build/initramfs.list"
    return
  fi
  if [[ -f "$ROOTFS/initramfs.list" ]]; then
    echo "$ROOTFS/initramfs.list"
    return
  fi
  die "missing rootfs initramfs list under $ROOTFS"
}

build_rootfs() {
  require_file "$ROOTFS/Makefile"
  make -C "$ROOTFS" -j"$JOBS"
  require_file "$(rootfs_initramfs_list)"
}

rewrite_linux_build_makefile() {
  local build=$1
  local makefile="$build/Makefile"
  [[ -f "$makefile" ]] || return 0
  LINUX_SRC="$LINUX_SRC" perl -pi -e '
    s|^MAKEARGS := -C .*$|MAKEARGS := -C $ENV{LINUX_SRC}|;
    s|^# Automatically generated by .*/scripts/mkmakefile:|# Automatically generated by $ENV{LINUX_SRC}/scripts/mkmakefile:|;
  ' "$makefile"
}

set_linux_initramfs_source() {
  local build=$1
  local initramfs=$2
  [[ -f "$build/.config" ]] &&
    INITRAMFS_LIST="$initramfs" perl -pi -e \
      's|^CONFIG_INITRAMFS_SOURCE=.*$|CONFIG_INITRAMFS_SOURCE="$ENV{INITRAMFS_LIST}"|' \
      "$build/.config"
  [[ -f "$build/include/config/auto.conf" ]] &&
    INITRAMFS_LIST="$initramfs" perl -pi -e \
      's|^CONFIG_INITRAMFS_SOURCE=.*$|CONFIG_INITRAMFS_SOURCE="$ENV{INITRAMFS_LIST}"|' \
      "$build/include/config/auto.conf"
  [[ -f "$build/include/generated/autoconf.h" ]] &&
    INITRAMFS_LIST="$initramfs" perl -pi -e \
      's|^#define CONFIG_INITRAMFS_SOURCE .*$|#define CONFIG_INITRAMFS_SOURCE "$ENV{INITRAMFS_LIST}"|' \
      "$build/include/generated/autoconf.h"
}

prepare_linux_build_tree() {
  local build=$1
  local initramfs=$2
  require_file "$build/Makefile"
  rewrite_linux_build_makefile "$build"
  set_linux_initramfs_source "$build" "$initramfs"
}
...
linux_relink_impl() {
  ...
  require_dir "$HOST_SHIM"
  [[ -n "$AVR32_TOOLS" ]] || die "AVR32_TOOLS was not detected"
  [[ -n "$COREUTILS" ]] || die "coreutils gnubin directory was not detected"
  mkdir -p "$OUT_DIR"
  build_rootfs
  initramfs=$(rootfs_initramfs_list)
  prepare_linux_build_tree "$LINUX_LLVM_BUILD" "$initramfs"
  if [[ -d "$LINUX_GCC_BUILD" ]]; then
    prepare_linux_build_tree "$LINUX_GCC_BUILD" "$initramfs"
  fi
